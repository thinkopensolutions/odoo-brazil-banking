...
class PaymentOrder(models.Model):
    ...
    def validate_order(self):
        if not len(self.line_ids):
            raise Warning(_("Please select lines to export"))
        # code must belong to one of allowed code
        if self.mode_type.code not in ['240', '400', '500']:
            raise Warning(_("Payment Type Code must be 240, 400 or 500, found %s" % self.mode_type.code))
        # legal name max length is accepted 30 chars
        if len(self.company_id.legal_name) > 30:
            raise Warning(_("Company's Rezão Social should not be longer than 30 chars"))
        if not self.mode.boleto_protesto:
            raise Warning(_(u"Códigos de Protesto in payment mode not defined"))
        if not self.mode.boleto_protesto_prazo:
            ...
            try:
                ...
                raise Warning(_("Prazo protesto in payment mode must be integer"))
        if not self.mode.bank_id.bra_number_dig:
            raise Warning(_("Dígito Agência not defined"))
        else:
            try:
        ...
        for line in self.line_ids:
            ...
            if not line.partner_id.state_id:
                raise Warning(_("Partner's state not defined"))
            if not line.partner_id.state_id.code:
                raise Warning(_("Partner's state code not defined"))
            # max 15 chars
            if not line.partner_id.district:
                raise Warning(_("Partner's bairro not defined"))
            if not line.partner_id.zip:
                raise Warning(_("Partner's CEP not defined"))
            if not line.partner_id.l10n_br_city_id:
                raise Warning(_("Partner's city not defined"))
            if not line.partner_id.street:
                raise Warning(_("Partner's street not defined"))
            if not line.move_line_id.transaction_ref:
                raise Warning(_("No transaction reference set for move %s" % line.move_line_id.name))
